...
from conv_vae import SingleScaleConvVAE  # 修改为你的无条件VAE文件名和类名
from conv_vae import loss_function_singlescale
...
start_epoch = 0
checkpoint_path = f'conv_vae_{version}.pth'
best_fid = float('inf')
...
for epoch in range(start_epoch, num_epochs):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        recon_x, mu_4x4, logvar_4x4, h_4x4 = model(noisy_data)
        loss = loss_function_singlescale(
            recon_x, data, mu_4x4, logvar_4x4, h_4x4)
        loss.backward()
    ...
    if (epoch + 1) % 10 == 0:
        ...
        with torch.no_grad():
            ...
            if fid_score < best_fid:
                best_fid = fid_score
                torch.save(model.state_dict(), f'conv_vae_{version}_best.pth')
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f'conv_vae_{version}_loss.png')
...
if len(fid_list) > 0:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'conv_vae_{version}_fid.png')
