...
from conv_cvae_model import ConvCVAE
...
model.load_state_dict(torch.load(f'conv_cvae_{version}_best.pth', map_location=device))
model.eval()
print("模型权重已加载")
...
with torch.no_grad():
    # 编码
    mu, logvar = model.encode(imgs, labels)
    z = model.reparameterize(mu, logvar)
    recon = model.decode(z, labels).cpu()

    # 采样生成
    z_sample = torch.randn(imgs.size(0), model.latent_dim).to(device)
    gen = model.decode(z_sample, labels).cpu()
